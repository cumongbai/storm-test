...
public class RedisHelper implements Runnable{
	...
	public void statics(String prefix)
	{
		...
		for(String tmpAppDataKey: appDataKeys)
		{
			...
			//已在在，添加到历史数据map中，序列化map
			if(tmpHisAppDataValue != null){
				hisAppDataValue = (Map<String, Integer>)SerializeUtil.unserialize(tmpHisAppDataValue);
			...
			//历史数据value,{1201:10,...}
			currKeyValues[counter++] = String.valueOf(SerializeUtil.serialize(hisAppDataValue));
	...
	public void shootShow()
	{
		...
		for(String tmpAppDataKey: appDataKeys)
		{
			hisAppDataValue = (Map<String, Integer>)SerializeUtil.unserialize(hisAppData.get(tmpAppDataKey));
